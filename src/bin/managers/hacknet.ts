...
export async function main(ns: NS): Promise<void> {
    ...
    function productionDifference(currentStats: NodeStats, levelUpgrades: number, ramUpgrades: number, coreUpgrades: number) {
        ...
        const afterUpgradeProduction = ns.formulas.hacknetNodes.moneyGainRate(currentStats.level + levelUpgrades, Math.pow(2, (Math.log(currentStats.ram) / Math.log(2)) + ramUpgrades), currentStats.cores + coreUpgrades, HACKNET_PRODUCTION_MULTIPLIER)
